...
class GitConfigHelper:
    ...
    @staticmethod
    def check_quick_auth() -> bool:
        """
        Checks if quick authentication is enabled in any config file.
        Returns True if quick auth is enabled, False otherwise.
        """
        getters = [
            (GitConfigLevel.SYSTEM, GitConfig.get_system_config),
            (GitConfigLevel.XDG, GitConfig.get_xdg_config),
        ...
        for _dummy, getter in getters:
            try:
                config = getter()
                with _suppress(KeyError):
                    quick_auth = config["user.quickauth"]
                    if quick_auth.lower() == "true":
                        return True
            except OSError:
                continue
