...
class MainWindow(QMainWindow):
    ...
    def fillGlobalMenuBar(self):
        ...
        ActionDef.addToQMenu(
            ...
            ActionDef(_("Git &Identity…"), self.editGitIdentity,
                      icon="user-identity",
                      tip=_("Set up your Git identity and authentication")),
    ...
    def editGitIdentity(self):
        """Open the Git Identity dialog to edit name, email and PAT credentials"""
        try:
            rw = self.currentRepoWidget()
            tasks.SetUpGitIdentity.invoke(rw, '', False)
        except NoRepoWidgetError:
            # If no repo is open, run the task without a repository context
            tasks.SetUpGitIdentity.runStandalone(parent=self)
